(function () {
    ...
    function runConnectLogic() {
        ...
        try {
            if (window.localStorage.getItem("connect-app-id") === "aspera-connect-container") {
                window.localStorage.removeItem("connect-app-id");
            }
        } catch (ignored) {
            // Connect can still initialize when browser storage is unavailable.
        }
