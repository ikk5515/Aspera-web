...
function Invoke-AllTrackedSecretScan {
    $previousErrorPreference = $ErrorActionPreference
    try {
        $ErrorActionPreference = 'Continue'
        $script:lastSecretScanOutput = @(& $powerShellExecutable -NoLogo -NoProfile -File `
            (Join-Path $repositoryRoot 'scripts/check-secrets.ps1') 2>&1)
        return $LASTEXITCODE
    } finally {
        $ErrorActionPreference = $previousErrorPreference
    }
...
try {
    ...
    Set-SyntheticIndexEntry -Path 'scripts/synthetic-index-only.conf' `
        -Content ($nodeKeyPrefix + 'USERNAME=sa')
    if ((Invoke-AllTrackedSecretScan) -eq 0) {
        throw 'Secret scanner skipped a tracked blob that was absent from the worktree.'
    }
    ...
    Write-Host "Secret scanner regression tests passed ($($negativeCases.Count + 3) cases)."
} finally {
    Pop-Location
